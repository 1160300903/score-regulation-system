/*本程序特色：算学分绩并按学分绩排序，查找成绩区间上的所有同学，删除学生*/ 
#include<stdio.h>
#include<stdlib.h>
#include<string.h>
...
void  ReadScore(STU *stu,int *m)
{
	...
	do{
 		...
 		scanf("%d",m);
 		fflush(stdin);
	 }while(!(*m<=6&&*m>0));/*确保输入数据正确*/ 
	printf("Input student's ID, name and score:\n"); 
	for( ;stu!=NULL;stu=stu->next)/*读入数据*/ 
	{
...
void  SortbyScore(STU *stu,int (*compare)(float a,float b))
{
	...
	{
		temp=i;
		for(j=i->next;j!=NULL;j=j->next)/*寻找最大值*/ 
		{
...
{
	STU *i,*j,*temp;
	for(i=stu;i->next!=NULL;i=i->next)/*按学号进行排序*/ 
	{
		temp=i;
		for(j=i->next;j!=NULL;j=j->next)/*寻找最大值*/
		if(j->num<temp->num)
...
{
	STU *i,*j,*temp;
	for(i=stu;i->next!=NULL;i=i->next)/*按姓名排序*/ 
	{
		temp=i;
		for(j=i->next;j!=NULL;j=j->next)/*寻找最大值*/
...
void  SearchbyName(STU *stu, int m)
{
	...
	for( ;stu!=NULL;stu=stu->next)
	{
		if(strcmp(stu->name,x)==0)/*判断是否是目标*/ 
		{
...
void  ReadfromFile(STU *stu,int *n, int *m)
{
	...
	else
	{
		...
		for(i=0;i<*n;i++)
		{
			fscanf(fp, "%ld%s\t", &stu->num, stu->name);
			for(j=0;j<*m;j++)
			{
...
{  
    ...
    {  
    	...
    	{  
        	if (strcmp(node1->name,name)==0) /*如果头节点为目标*/ 
        	{  
            	head=head->next;  
            	free(node1);  
            	return head;  
    ...
    else if(strcmp(a,"num")==0)/*如果为学号删除*/ 
    {
...
{
	STU *head=stu;
	int lower=0,upper=-1,j,i=0;/*lower,upper分别为删除成绩的上下线*/ 
	SortbyScore(stu,Descending);
	for( ;head!=NULL;head=head->next)/*先求的总成绩*/ 
	{
		head->sum=0;
		for(j=0;j<m;j++)
		{
	...
	while(upper<lower)/*保证上限比下限大，数据输入正确*/ 
	{
	printf("Please enter the Interval upper limit and Interval lower limit(lower can be equal to upper)(sum score is %d)\n",100*m);
...
void CreditGrade(STU *stu,int m)
{
	...
	for(i=0;i<m;i++)
	{
		...
		if(strcmp(a,"y")==0||strcmp(a,"Y")==0)/*读入考试课对应学分*/ 
		{
		countc++; 
	...
	for( ;stu!=NULL;stu=stu->next)/*计算每个学生的学分绩*/ 
	{
		stu->creditgrade=0;
		for(i=0;i<m;i++)
		{
	...
	for( ;pi!=NULL;pi=pi->next)/*对学生学分绩排名*/ 
	{
		k=pi;
